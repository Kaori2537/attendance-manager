...
export async function resumeWorkWithTasks(plannedTasks: Task[]) {
  ...
  try {
    ...
    if (!dbRes.ok) {
      ...
      throw new Error(`Database break-end failed: ${dbRes.status} ${text}`);
    }

    // 2) DB: daily-reports upsert（追加タスク）- タスクがある場合のみ
    ...
    console.error("resumeWorkWithTasks Error:", err);
